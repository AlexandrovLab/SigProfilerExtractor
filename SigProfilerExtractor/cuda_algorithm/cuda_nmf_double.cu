...
__global__ void sumCommSingleBlock(const dtype *a, dtype *out, const int arraySize) {
    int idx = threadIdx.x;
...
void NMFHandleCreate(
    ...
    // Local temporary storage -- this is free'd at the end of this function
    //
    ...
    // d_sum
    cudaMalloc((void **) & nmf_handle->d_sum, sizeof(dtype));
    // d_Vsum
    cudaMalloc((void **) & nmf_handle->d_Vsum, sizeof(dtype));
    // d_WHsum
    cudaMalloc((void **) & nmf_handle->d_WHsum, sizeof(dtype));
...
int fit_loop(
    ...
    for(int iter=0; iter < max_iterations / test_conv; iter++) {
        ...
        // copy sum to host
        cudaMemcpy((&sum), nmf_handle->d_sum, sizeof(dtype), cudaMemcpyDeviceToHost);
        // copy Vsum to host
        cudaMemcpy((&Vsum), nmf_handle->d_Vsum, sizeof(dtype), cudaMemcpyDeviceToHost);
        // copy WHsum to host
        cudaMemcpy((&WHsum), nmf_handle->d_WHsum, sizeof(dtype), cudaMemcpyDeviceToHost);
...
PYBIND11_MODULE(cuda_nmf_double, cudaBind) {
