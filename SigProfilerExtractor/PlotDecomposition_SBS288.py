...
def gen_plot(de_novo_name, basis_names, output_path, project, c, reconstruction, statistics, sig_version, custom_text):

    # THIS IS THE RIGHT SIDE, IT CHANGES
	num_bases = len(basis_names)
	if (num_bases == 1):
		plot_1(basis_names, output_path, project, c)
	elif (num_bases == 2):
		plot_2(basis_names, output_path, project, c)
	elif (num_bases == 3):
		plot_3(basis_names, output_path, project, c)
	elif (num_bases == 4):
		plot_4(basis_names, output_path, project, c)
	elif (num_bases == 5):
		plot_5(basis_names, output_path, project, c)
	elif (num_bases > 5):
		plot_6_plus(basis_names, output_path, project, c)

	# THIS IS THE LEFT SIDE
	if reconstruction:
		recon_png = output_path+"/SBS_sub_plots/"+de_novo_name+"_reconstruction_"+project+".png"
		c.drawImage(output_path+"/SBS_sub_plots/"+de_novo_name+"_"+project+".png", WIDTH_GAP, \
			LAYOUT_2_GRAPH[0][Y_COORD], width=WIDTH_GRAPH, height=HEIGHT_GRAPH)
		c.drawString(WIDTH_GRAPH - WIDTH_GAP - 25, LAYOUT_2_TEXT[0][Y_COORD] + 15, "Original")
		...
		c.drawImage(output_path+ "/SBS_sub_plots/" +de_novo_name+"_"+project+".png", \
